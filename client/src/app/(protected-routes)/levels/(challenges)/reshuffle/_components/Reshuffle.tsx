...
function Reshuffle() {
  ...
  > = useQuery({
    queryKey: ["reshuffle-score"],
    queryFn: async () => {
      const response = await axiosInterceptor.get(
        `${baseUrl}/feature/reshuffle-score`,
        {
          headers: {
    ...
    <div className="w-full h-full flex flex-col py-5 justify-center items-center  px-5">
      <div className="w-full sm:w-[500px] min-h-[400px] flex flex-col p-2.5 space-y-5 relative items-center">
        ...
        <div>
          ...
          >
            The cards will automatically reshuffle everytime the user matched
            the two cards.
          </p>
        </div>
        <div className="flex flex-col space-y-4">
          <div className="flex flex-col pt-5">
            ...
            >
              {getScore.data?.personalReshuffleScore?.highScore}
            </small>
          </div>
          <div className="flex flex-col">
            ...
            <small
              ...
              className="text-white text-center"
            >
              {getScore.data?.personalReshuffleScore?.totalScore}
            </small>
          </div>
          <div className="flex flex-col">
        ...
        <div className="flex w-full justify-center items-center space-x-2 pt-5">
          ...
          <motion.button
            onClick={() => {
              router.push("/levels/reshuffle/play");
              playClickSound();
